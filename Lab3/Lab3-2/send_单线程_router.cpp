...
bool Send(SOCKET& send_socket, SOCKADDR_IN& recv_addr, int& recv_addrlen, char* message, int msg_len) {
	...
	int head = 0;  // 已发送未确认的头部
	int tail = (times > WINDOWS) ? WINDOWS : times;  // 尾部
	int now = 0;  // 发送到的下标
	...
	// 窗口右移，直到结束
	while (head < times) {
		cout << "发送窗口" << "  head: " << head << "  tail: " << tail << endl;
		// 操作当前窗口，循环发送head到tail的内容
		for (now; now < tail; now++) {
			cout << "传输now: " << now << endl;
			cout << "传输序列号SEQ为 " << now % 256 << endl;
			// 计算本次传输数据大小
		...
		cout << "本次窗口中内容发出完毕" << endl;
		
		clock_t start = clock();  // 记录窗口发送时间
		...
		while (true) {
			...
			if (recvlength > 0) {
				...
				if (checksum(header2) != 0) {
					...
					// 收到重复ACK，重发窗口
					if ((int)header2.ack == (head % 256)) {
						cout << "收到重复ack：" << (int)header2.ack << "  重发窗口！！" << endl;
						now = head;
						break;
					}
					else {  // 更新窗口 
						int step = ((int)header2.ack - (head % 256) + 256) % 256;
						cout << "收到正确ack：" << (int)header2.ack << "  窗口更新，右移" << step << endl;

						head += step;
						tail = (tail + 1 > times) ? times : (tail + 1);
						break;
					}
				}
			}
